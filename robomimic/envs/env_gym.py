...
class EnvGym(EB.EnvBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.camera_names = kwargs.get("camera_names", None)
        self.imsize = kwargs.get("imsize", None)

        if "camera_names" in kwargs:
            del kwargs["camera_names"]
        if "imsize" in kwargs:
            del kwargs["imsize"]
    ...
    def reset_to(self, state):
        ...
        if hasattr(self.env.unwrapped.sim, "set_state_from_flattened"):
            ...
            return self.get_observation(obs=self.env.unwrapped._get_obs())
        else:
    ...
    def get_observation(self, obs=None):
        ...
        obs_dict = { "flat" : np.copy(obs), "dataset_id" : np.ones((10,))}

        # hack for kitchen: use first 9 dims for robot
        obs_dict["robot_joints"] = obs_dict["flat"][:9]

        if self.use_image_obs:
            for cam_name in self.camera_names:
                im = self.env.render(mode="rgb_array", height=self.imsize, width=self.imsize, camera_name=cam_name)
                obs_key = "{}_image".format(cam_name)
                if self.postprocess_visual_obs:
                    im = ObsUtils.process_obs(obs=im, obs_key=obs_key)
                obs_dict[obs_key] = im
    ...
    def create_for_data_processing(cls, env_name, camera_names, camera_height, camera_width, reward_shaping, **kwargs):
        ...
        assert camera_width == camera_height
        new_kwargs = {
            "camera_names": list(camera_names),
            "imsize": camera_height,
        }
        ...
        return cls(
            env_name=env_name,
            use_image_obs=(len(camera_names) > 0),
