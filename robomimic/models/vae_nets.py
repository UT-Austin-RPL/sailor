...
class VAE(torch.nn.Module):
    ...
    def __init__(
        ...
        prior_condition_first_last=False,
        prior_layer_dims=(),
        prior_use_gmm=False,
        ...
        if len(self.condition_shapes) > 0:
            ...
            self.prior_condition_first_last = prior_condition_first_last
    ...
    def _create_prior(self):
        ...
        if self._is_cvae:
            if self.prior_is_conditioned:
                ...
                if self.prior_condition_first_last:
                    assert not self._is_goal_conditioned
                    prior_obs_group_shapes["goal"] = prior_obs_group_shapes["condition"]
            if self._is_goal_conditioned:
    ...
    def kl_loss(self, posterior_params, encoder_z=None, conditions=None, goals=None):
        ...
        if self.prior_is_conditioned:
            if self.prior_condition_first_last:
                goals = TensorUtils.index_at_time(conditions, ind=-1)
                conditions = TensorUtils.index_at_time(conditions, ind=0)
            else:
                conditions = TensorUtils.index_at_time(conditions, ind=0) # should we use full history instead?
...
class VAE_RNN(VAE):
    """
    An extension to the VAE class used to model distributions over sequential data.
    The encoder, decoder, and prior may all be RNNs. The latent space can either
    represent entire sequences, or still correspond to single data points.
    """
    def __init__(
            self,
            input_shapes,
            ...
            decoder_is_conditioned=True,
            decoder_reconstruction_sum_across_elements=False,
            # decoder_learn_variance=False,
            # decoder_use_batch_variance=False,
            latent_clip=None,
            output_squash=(),
            output_scales=None,
            output_ranges=None,
            ...
            prior_gmm_num_modes=10,
            prior_gmm_learn_weights=False,
            # prior_gmm_weights_are_conditioned=False,
            # prior_gmm_low_noise_eval=False,
            prior_use_categorical=False,
            prior_categorical_dim=10,
            prior_categorical_gumbel_softmax_hard=False,
            # prior_use_normalizing_flow=False,
            # prior_flow_layer_dims=(),
            # prior_flow_num=2,
            # prior_flow_activation=nn.ReLU,
            goal_shapes=None,
            encoder_kwargs=None,
            # goal_share_encoder=True,
            ...
            rnn_decoder=True,
            rnn_prior=False,
            latent_per_step=False,
    ):
        """
        Args (specific to VAE_RNN - see VAE class for others):
            rnn_hidden_dim (int): RNN hidden dimension
            rnn_num_layers (int): number of RNN layers
            rnn_type (str): [LSTM, GRU]
            rnn_kwargs (dict): kwargs for the torch.nn.LSTM / GRU
            rnn_encoder (bool): if True, encoder is an RNN
            rnn_decoder (bool): if True, decoder is an RNN
            rnn_prior (bool): if True, prior is an RNN
            latent_per_step (bool): if True, a new latent distribution is generated
                from the encoder and the prior at each step of the sequence. Otherwise,
                a single latent distribution is generated for the entire sequence. If
                this is False, and the encoder and prior are not RNNs, the encoder
                and prior will generate the latent distribution from the first timestep
                of each input sequence.
        """
        self.rnn_hidden_dim = rnn_hidden_dim
        self.rnn_num_layers = rnn_num_layers
        self.rnn_type = rnn_type
        self.encoder_rnn_kwargs = deepcopy(rnn_kwargs)
        self.decoder_rnn_kwargs = deepcopy(rnn_kwargs)
        self.prior_rnn_kwargs = deepcopy(rnn_kwargs)
        # only the encoder is allowed to be bidirectional (decoder and prior need to be causal)
        if "bidirectional" in self.decoder_rnn_kwargs:
            self.decoder_rnn_kwargs.pop("bidirectional")
            self.prior_rnn_kwargs.pop("bidirectional")
        self.encoder_is_rnn = rnn_encoder
        self.decoder_is_rnn = rnn_decoder
        self.prior_is_rnn = rnn_prior
        self.latent_per_step = latent_per_step

        assert self.encoder_is_rnn or self.decoder_is_rnn or self.prior_is_rnn
        ...
        super(VAE_RNN, self).__init__(
            ...
            # decoder_learn_variance=decoder_learn_variance,
            # decoder_use_batch_variance=decoder_use_batch_variance,
            ...
            prior_condition_first_last=prior_condition_first_last,
            ...
            # prior_gmm_weights_are_conditioned=prior_gmm_weights_are_conditioned,
            # prior_gmm_low_noise_eval=prior_gmm_low_noise_eval,
            ...
            # prior_use_normalizing_flow=prior_use_normalizing_flow,
            # prior_flow_layer_dims=prior_flow_layer_dims,
            # prior_flow_num=prior_flow_num,
            # prior_flow_activation=prior_flow_activation,
            # share_visual_weights=share_visual_weights,
            goal_shapes=goal_shapes,
            encoder_kwargs=encoder_kwargs,
            # goal_share_encoder=goal_share_encoder,
        )
    ...
    def _create_encoder(self):
        ...
        Helper function to create encoder.
        """
        if not self.encoder_is_rnn:
            # superclass makes flat encoder
            return super(VAE_RNN, self)._create_encoder()
        ...
        if self.latent_per_step:
            # compute posterior at each step
            per_step = True
        else:
            # only last RNN hidden state used to compute posterior
            per_step = False

        self.nets["encoder"] = RNN_MIMO_MLP(
            input_obs_group_shapes=encoder_obs_group_shapes,
            output_shapes=encoder_output_shapes,
            mlp_layer_dims=self.encoder_layer_dims,
            rnn_hidden_dim=self.rnn_hidden_dim,
            rnn_num_layers=self.rnn_num_layers,
            rnn_type=self.rnn_type,
            rnn_kwargs=self.encoder_rnn_kwargs,
            per_step=per_step,
            encoder_kwargs=self._encoder_kwargs,
        )
    ...
    def _create_decoder(self):
        ...
        Helper function to create decoder.
        """
        if not self.decoder_is_rnn:
            # superclass makes flat decoder
            return super(VAE_RNN, self)._create_decoder()
        ...
        decoder_obs_group_shapes["input"] = OrderedDict(latent=latent_shape)
        if self._is_cvae:
            if self.decoder_is_conditioned:
                decoder_obs_group_shapes["condition"] = OrderedDict(self.condition_shapes)
            if self._is_goal_conditioned:
                decoder_obs_group_shapes["goal"] = OrderedDict(self.goal_shapes)

        self.nets["decoder"] = RNN_MIMO_MLP(
            input_obs_group_shapes=decoder_obs_group_shapes,
            output_shapes=self.output_shapes,
            mlp_layer_dims=self.decoder_layer_dims,
            rnn_hidden_dim=self.rnn_hidden_dim,
            rnn_num_layers=self.rnn_num_layers,
            rnn_type=self.rnn_type,
            rnn_kwargs=self.decoder_rnn_kwargs,
            encoder_kwargs=self._encoder_kwargs,
        )

        # if self.decoder_learn_variance and not self.decoder_use_batch_variance:
        #     self.decoder_log_sigma = torch.nn.Parameter(torch.zeros(1)[0])
        #     # self.nets["decoder_log_sigma"] = nn.ModuleDict()
        #     # for k in self.output_shapes:
        #     #     self.nets["decoder_log_sigma"][k] = torch.nn.Parameter(torch.zeros(1)[0])

    def _create_prior(self):
        """
        Helper function to create prior.
        """
        if not self.prior_is_rnn:
            # superclass makes flat decoder
            return super(VAE_RNN, self)._create_prior()

        raise NotImplementedError

    def get_rnn_init_state(self, batch_size, device):
        """
        Get a default RNN state from each component that is an RNN.
        ...
        """
        rnn_init_state = dict()
        if self.encoder_is_rnn:
            rnn_init_state["encoder"] = self.nets["encoder"].get_rnn_init_state(batch_size, device=device)
        if self.prior_is_rnn:
            rnn_init_state["prior"] = self.nets["prior"].prior_module.get_rnn_init_state(batch_size, device=device)
        if self.decoder_is_rnn:
            rnn_init_state["decoder"] = self.nets["decoder"].get_rnn_init_state(batch_size, device=device)
        return rnn_init_state

    def encode(self, inputs, conditions=None, goals=None, rnn_init_state=None, return_state=False):
        """
        Args:
            inputs (dict): a dictionary that maps input modalities to torch.Tensor
                batches. These should correspond to the encoder-only modalities
                (i.e. @self.encoder_only_shapes). Each modality should have leading
                dimensions [B, T] corresponding to batch size and sequence length.
            conditions (dict): a dictionary that maps modalities to torch.Tensor
                batches. These should correspond to the modalities used for conditioning
                in either the decoder or the prior (or both). Only for cVAEs. Each
                modality should have leading dimensions [B, T] corresponding to
                batch size and sequence length.
            goals (dict): a dictionary that maps modalities to torch.Tensor
                batches. These should correspond to goal modalities. Only for cVAEs.
            rnn_init_state (dict): optional - should be a dictionary with keys "encoder", "decoder"
                and "prior" and the corresponding hidden states, for each component that is an RNN
            return_state (bool): if True, returns a dictionary with key "encoder" and the
                corresponding hidden state. If encoder is not an RNN, returns None.
        Return:
            posterior params (dict): dictionary with posterior parameters. If
                @latent_per_step is True, sequences are returned for each parameter
                (e.g. leading dimensions [B, T]).
            rnn_state (dict): dictionary with "encoder" key for the new hidden state.
                Only returned if @return_state is True.
        """
        if not self.encoder_is_rnn:
            if self.latent_per_step:
                # compute encoder outputs independently per timestep and distribute across
                # time by reshaping from [B, T, ...] to [B * T, ...], doing forward pass,
                # and then reshaping outputs back to [B, T, ...]
                if self._is_cvae:
                    op = lambda x, y : self.nets["encoder"](input=x, condition=y, goal=goals)
                    td_inputs = dict(x=inputs, y=conditions)
                else:
                    op = lambda x : self.nets["encoder"](input=x, condition=conditions, goal=goals)
                    td_inputs = dict(x=inputs)
                posterior_params = TensorUtils.time_distributed(
                    inputs=td_inputs,
                    op=op,
                    inputs_as_kwargs=True,
                )
            else:
                # # just use the first timestep of each input seqeunce
                # inputs = TensorUtils.index_at_time(inputs, 0)
                # if self._is_cvae:
                #     conditions = TensorUtils.index_at_time(conditions, 0)
                
                posterior_params = self.nets["encoder"](input=inputs, condition=conditions, goal=goals)

            if return_state:
                # caller expects a hidden state returned, but encoder is not an RNN
                assert rnn_init_state is None or "encoder" not in rnn_init_state
                return posterior_params, None
            return posterior_params

        # extract "encoder" part of init state if it exists, or default to None (to create new state)
        encoder_rnn_init_state = rnn_init_state.get("encoder", None) if rnn_init_state is not None else None

        out = self.nets["encoder"].forward(
            input=inputs,
            condition=conditions,
            goal=goals,
            rnn_init_state=encoder_rnn_init_state,
            return_state=return_state,
        )
        if return_state:
            out, new_encoder_rnn_state = out
            return out, dict(encoder=new_encoder_rnn_state)
        return out
    ...
    def reparameterize(self, posterior_params):
        ...
        """
        if self.latent_per_step:
            # time-distribute the reparametrization of each latent at each timestep
            return TensorUtils.time_distributed(
                inputs=posterior_params,
                op=super(VAE_RNN, self).reparameterize,
            )
        return super(VAE_RNN, self).reparameterize(posterior_params)

    def decode(self, conditions=None, goals=None, z=None, n=None, t=None, rnn_init_state=None, return_state=False):
        """
        Pass latents through decoder. Latents should be passed in to
        this function at train-time for backpropagation, but they
        can be left out at test-time. In this case, latents will
        be sampled using the VAE prior (and may also be clipped).
        Args:
            conditions (dict): a dictionary that maps modalities to torch.Tensor
                batches. These should correspond to the modalities used for conditioning
                in either the decoder or the prior (or both). Only for cVAEs.
            goals (dict): a dictionary that maps modalities to torch.Tensor
                batches. These should correspond to goal modalities. Only for cVAEs.
            z (torch.Tensor): if provided, these latents are used to generate
                reconstructions from the VAE. If @latent_per_step is True,
                then this should be a batch of sequences of shape [B, T, D],
                otherwise, it should be a batch of shape [B, D].
            n (int): if provided, this argument is used to specify the number
                of samples to generate from the VAE. Only required if @conditions
                is not provided (i.e. if this VAE is not a cVAE)
            t (int): if provided, this argument is used to specify the number
                of timesteps to decode from the VAE. Only required if @conditions
                is not provided (i.e. if this VAE is not a cVAE)
            rnn_init_state (dict): optional - should be a dictionary with keys "encoder", "decoder"
                and "prior" and the corresponding hidden states, for each component that is an RNN
            return_state (bool): if True, returns a dictionary with keys "decoder"
                and "prior" that has the corresponding hidden states of each. Note
                that the "prior" hidden state will only be returned if it is an RNN
                and @z is not provided (so prior sampling occurred). If neither the
                decoder nor the prior are RNNs, None will be returned.
        """
        need_prior_sample = (z is None)
        if z is None:
            # sample latents from prior distribution
            z = self.sample_prior(conditions=conditions, goals=goals, n=n, t=t, rnn_init_state=rnn_init_state, return_state=return_state)
            if return_state:
                z, new_prior_rnn_state = z

        # sequence length checking
        if self._is_cvae and self.decoder_is_conditioned:
            if (conditions is not None) and (t is not None):
                for mod in conditions:
                    assert conditions[mod].shape[1] == t, "conditions and t mismatch in @decode"

        if t is None:
            # infer sequence length from inputs
            assert conditions is not None
            mod = list(conditions.keys())[0]
            t = conditions[mod].shape[1]

        if not self.latent_per_step:
            # latent is constant for all timesteps - add time dimension and tile to prep for network
            z = TensorUtils.unsqueeze_expand_at(z, size=t, dim=1)
        assert z.shape[1] == t

        # modality group: modality inputs
        # decoder takes latents as input, and optionally is conditioned on
        # condition variables and goal variables
        inputs = dict(
            input=dict(latent=z),
            condition=conditions,
            goal=goals,
        ...
        # pass through decoder to reconstruct variables in @self.output_shapes
        if self.decoder_is_rnn:
            # extract "decoder" part of init state if it exists, or default to None (to create new state)
            decoder_rnn_init_state = rnn_init_state.get("decoder", None) if rnn_init_state is not None else None
            recons = self.nets["decoder"].forward(rnn_init_state=decoder_rnn_init_state, return_state=return_state, **inputs)
            if return_state:
                recons, rnn_state = recons
        else:
            # compute reconstructions independently per timestep and distribute across
            # time by reshaping from [B, T, ...] to [B * T, ...], doing forward pass,
            # and then reshaping outputs back to [B, T, ...]
            if self._is_cvae and self.decoder_is_conditioned:
                op = lambda x, y: self.nets["decoder"](input=x, condition=y, goal=inputs["goal"])
                td_inputs = dict(x=inputs["input"], y=inputs["condition"])
            else:
                op = lambda x: self.nets["decoder"](input=x, condition=inputs["condition"], goal=inputs["goal"])
                td_inputs = dict(x=inputs["input"])
            recons = TensorUtils.time_distributed(
                inputs=td_inputs,
                op=op,
                inputs_as_kwargs=True,
            )
        ...
        if return_state:
            ...
            if self.decoder_is_rnn:
                state_to_return["decoder"] = rnn_state
            if self.prior_is_rnn and need_prior_sample:
                # only include prior hidden state if we sampled from prior
                state_to_return["prior"] = new_prior_rnn_state["prior"]
            if len(state_to_return) == 0:
                # return None if no RNN hidden states are present
                state_to_return = None
            return recons, state_to_return
        return recons

    def decode_step(self, conditions=None, goals=None, z=None, n=None, t=None, rnn_state=None):
        """
        Run through the VAE decoder for a single RNN step.
        """
        assert self.decoder_is_rnn
        if conditions is not None:
            conditions = TensorUtils.to_sequence(conditions)
        outputs, rnn_state = self.decode(conditions=conditions, goals=goals, z=z, n=n, t=t, rnn_init_state=rnn_state, return_state=True)
        return {k: outputs[k][:, 0] for k in outputs}, rnn_state

    def sample_prior(self, conditions=None, goals=None, n=None, t=None, rnn_init_state=None, return_state=False):
        """
        Update from superclass to have support for RNN prior.
        Args:
            rnn_init_state (dict): optional - should be a dictionary with keys "encoder", "decoder"
                and "prior" and the corresponding hidden states, for each component that is an RNN
            return_state (bool): if True, returns a dictionary with key "prior" and the corresponding
                hidden state. If the prior is not an RNN, returns None.
        """

        # maybe infer number of samples and sequence length from inputs
        if conditions is not None:
            mod = list(conditions.keys())[0]
            n_cond, t_cond = conditions[mod].shape[:2]
            if n is None:
                n = n_cond
            if t is None:
                t = t_cond
        ...
        if self.latent_per_step:
            if not self.prior_is_rnn:
                # prior is flat and inputs have leading dimension [B, T], so time-distribute by reshaping to [B * T]
                if conditions is not None:
                    conditions = TensorUtils.join_dimensions(conditions, 0, 1)
            else:
                # time-distribute N * T independent samples for latent sequence
                assert (n is not None)
                n_sample = n * t
        else:
            assert not self.prior_is_rnn
            # latents have no temporal dimension - just take first timestep of sequences
            if conditions is not None:
                goals = TensorUtils.index_at_time(conditions, ind=-1)
                conditions = TensorUtils.index_at_time(conditions, ind=0)
                return self.nets["prior"].sample(n=n_sample, obs_dict=conditions, goal_dict=goals)

        # condition and goal modality groups
        inputs = dict(condition=conditions, goal=goals)

        # prior network expects raw hidden state, not a dictionary
        init_state = rnn_init_state.get("prior", None) if rnn_init_state is not None else None

        samples = self.nets["prior"].sample(inputs=inputs, n=n_sample, rnn_init_state=init_state, return_state=return_state)
        if return_state:
            samples, new_state = samples

        if self.latent_per_step and not self.prior_is_rnn:
            # we time-distributed sample computation - now reshape [N * T, ...] -> [N, T, ...]
            samples = samples.reshape(n, t, *samples.shape[1:])

        if return_state:
            # turn raw hidden state into dict
            new_state = dict(prior=new_state) if new_state is not None else None
            return samples, new_state
        return samples

    def kl_loss(self, posterior_params, encoder_z=None, conditions=None, goals=None, rnn_init_state=None, return_state=False):
        """
        Conditioned prior will expect a single timestep for @conditions instead of a sequence,
        so account for that.
        Args:
            rnn_init_state (dict): optional - should be a dictionary with keys "encoder", "decoder"
                and "prior" and the corresponding hidden states, for each component that is an RNN
        ...
        if self.latent_per_step:
            if not self.prior_is_rnn:
                # prior is flat and inputs have leading dimension [B, T], so time-distribute by reshaping to [B * T]
                posterior_params = TensorUtils.join_dimensions(posterior_params, 0, 1)
                if encoder_z is not None:
                    encoder_z = TensorUtils.join_dimensions(encoder_z, 0, 1)
                if conditions is not None:
                    conditions = TensorUtils.join_dimensions(conditions, 0, 1)
        else:
            assert not self.prior_is_rnn
            ...
            if conditions is not None:
                # if self.prior_condition_first_last:
                #     conditions = TensorUtils.index_at_time(conditions, ind=[0, -1])
                # else:
                #     conditions = TensorUtils.index_at_time(conditions, ind=0)
                goals = TensorUtils.index_at_time(conditions, ind=-1)
                conditions = TensorUtils.index_at_time(conditions, ind=0)
        ...
        kl_loss = self.nets["prior"].kl_loss(
            ...
            obs_dict=conditions,
            goal_dict=goals,
            # rnn_init_state=init_state,
            # return_state=return_state,
        )
        if return_state:
            kl_loss, new_state = kl_loss
            # turn raw hidden state into dict
            return kl_loss, dict(prior=new_state)
        return kl_loss

    def forward(self, inputs, outputs, conditions=None, goals=None, rnn_init_state=None, freeze_encoder=False, return_state=False):
        """
        Modify from superclass to accept and return hidden states.
        Args (specific to this class):
            rnn_init_state (dict): optional - should be a dictionary with keys "encoder", "decoder"
                and "prior" and the corresponding hidden states, for each component that is an RNN
            return_state (bool): if True, returns a dictionary with keys "encoder", "decoder"
                and "prior" that has the corresponding hidden states of each
        """
        ...
        if return_state:
            posterior_params, encoder_updated_state = posterior_params
        ...
        if return_state:
            reconstructions, decoder_updated_state = reconstructions
            assert (decoder_updated_state is None) or ("prior" not in decoder_updated_state)
        ...
        if return_state:
            kl_loss, prior_updated_state = kl_loss
        ...
        if return_state:
            # return all states for RNN components
            rnn_state = dict()
            if self.encoder_is_rnn:
                rnn_state.update(encoder_updated_state)
            if self.decoder_is_rnn:
                rnn_state.update(decoder_updated_state)
            if self.prior_is_rnn:
                rnn_state.update(prior_updated_state)
            return vae_outputs, rnn_state
        return vae_outputs
