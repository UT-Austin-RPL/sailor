...
def get_generator(algo_name, config_file, args, algo_name_short=None):
    ...
    if args.script is not None:
        ...
        generated_config_dir=os.path.join(
            os.getenv("HOME"), 'tmp/autogen_configs/sailor', algo_name, args.env, args.mod, args.name, curr_time, "json",
        )
...
def set_num_seeds(generator, args):
    if args.seeds > 0:
        generator.add_param(
            ...
            name="seed",
            group=-10,
            values=[i + 1 for i in range(args.seeds)],
        )
