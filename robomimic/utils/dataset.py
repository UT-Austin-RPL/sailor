...
class SkillLearningDataset(SequenceDataset):
    def __init__(
        self,
        temp_dist_window_len,
        slowness_window_len,
        *args, **kwargs
    ):
        super(SkillLearningDataset, self).__init__(*args, **kwargs)
        assert self.pad_frame_stack

        self._temp_dist_window_len = temp_dist_window_len
        self._slowness_window_len = slowness_window_len

    def sample_index_within_window(self, index, window_len, offset=0):
        demo_id = self._index_to_demo_id[index]
        demo_start_index = self._demo_id_to_start_indices[demo_id]
        demo_length = self._demo_id_to_demo_length[demo_id]
        ...
        start = index + offset - window_len
        end = index + offset + window_len + 1

        start = np.clip(start, demo_start_index, demo_start_index + end_index_in_demo)
        end = np.clip(end, demo_start_index, demo_start_index + end_index_in_demo)
    ...
    def get_index_in_demo(self, index):
        demo_id = self._index_to_demo_id[index]
        demo_start_index = self._demo_id_to_start_indices[demo_id]

        return index - demo_start_index
    
    def get_item_wrapper(self, index):
        meta = dict()

        meta_curr = super(SkillLearningDataset, self).__getitem__(index)
        meta["t"] = np.array([self.get_index_in_demo(index)])
        meta.update(meta_curr)

        index_temp_dist = self.sample_index_within_window(index, self._temp_dist_window_len)
        meta_temp_dist = super(SkillLearningDataset, self).__getitem__(index_temp_dist)
        meta["t_temp_dist"] = np.array([self.get_index_in_demo(index_temp_dist)])
        for k in meta_temp_dist:
            meta['{}_temp_dist'.format(k)] = meta_temp_dist[k]

        index_slowness = self.sample_index_within_window(index, self._slowness_window_len)
        meta_slowness = super(SkillLearningDataset, self).__getitem__(index_slowness)
        meta["t_slowness"] = np.array([self.get_index_in_demo(index_slowness)])
        for k in meta_slowness:
            meta['{}_slowness'.format(k)] = meta_slowness[k]

        return meta

    def __getitem__(self, index):
        return self.get_item_wrapper(index)

class SAILOR_Dataset(torch.utils.data.Dataset):
    def __init__(
        self,
        ret_ds_path,
        target_ds_path,
        ...
        retrieval_model,
        filter_by_attribute=None,
        target_filter_key=None,
        num_ret_cands=250000,
        num_target_samples=2500,
        selection_frac=0.10,
        bs=None,
        num_workers=0,
        **kwargs
    ):
        self._model = retrieval_model

        obs_key_for_ret = list(
            set(obs_keys).union(set(self._model.obs_key_shapes.keys()))
        )
        if bs is None:
            bs = self._model.global_config.train.batch_size

        self._fs = kwargs.get('frame_stack', 1)

        self._ret_ds = SequenceDataset(
            hdf5_path=ret_ds_path,
            obs_keys=obs_key_for_ret,
            dataset_keys=dataset_keys,
            filter_by_attribute=filter_by_attribute,
            **kwargs
        )

        self._target_ds = SequenceDataset(
            hdf5_path=target_ds_path,
            obs_keys=obs_key_for_ret,
            dataset_keys=dataset_keys,
            filter_by_attribute=target_filter_key,
            **kwargs
        )

        dl_ret = DataLoader(
            dataset=self._ret_ds,
            sampler=None,
            batch_size=bs,
            shuffle=True,
        ...
        print("Getting zs...")
        # get zs and states
        ret_zs, ret_inds = self._get_encs(dl_ret, max_samples=num_ret_cands)
        target_zs, _ = self._get_encs(dl_target, max_samples=num_target_samples)
        print("done.\n")

        print("Calculating pairwise distances...")
        # get closest target state for each prior state
        pairwise_dists = cdist(ret_zs['mean'], target_zs['mean'])

        dists = np.min(pairwise_dists, axis=1)
        print("done.\n")

        # order prior segments by closest z distance to target data
        ret_inds = ret_inds[np.argsort(dists)]

        del pairwise_dists

        self._ret_inds = ret_inds[:int(len(ret_inds) * selection_frac)].astype(int)

        # delete datasets. reinitialize retrieval dataset with correct key
        del self._ret_ds
        del self._target_ds

        self._ret_ds = SequenceDataset(
            hdf5_path=ret_ds_path,
            obs_keys=obs_keys, # use obs_keys now, instead of obs_keys_superset
            dataset_keys=dataset_keys,
            filter_by_attribute=filter_by_attribute,
            **kwargs
        )

    def _get_encs(self, dl, max_samples):
        enc_list, ind_list = [], []
        num_samples = 0

        device = TorchUtils.get_torch_device(True)

        for batch in LogUtils.custom_tqdm(iter(dl)):
            batch = TensorUtils.to_device(TensorUtils.to_float(batch), device)

            with torch.no_grad():
                skill_len = self._model.skill_len

                # bunch of hacks here to get data
                actions = batch["actions"][:, :skill_len, :]
                obs_dict = {k: batch["obs"][k][:, self._fs - 1: self._fs - 1 + skill_len, :] for k in batch["obs"]}

                enc = self._model.nets["skill"].encode(actions, obs_dict=obs_dict)

            enc_list.append(TensorUtils.to_numpy(enc))
            ind_list.append(TensorUtils.to_numpy(batch['index']))

            num_samples += batch["actions"].shape[0]
            if (max_samples is not None) and (num_samples >= max_samples):
                break

        if isinstance(enc_list[0], dict):
            encs = {k: np.concatenate([enc[k] for enc in enc_list]) for k in enc_list[0].keys()}
        else:
            encs = np.concatenate(enc_list)
    ...
    def __len__(self):
        return len(self._ret_inds)

    def __getitem__(self, idx):
        return self._ret_ds.__getitem__(self._ret_inds[idx])

    def __repr__(self):
        str_output = self._ret_ds.__repr__()
        # a bit hacky: modify the original string
        str_output = '\n'.join(str_output.split('\n')[:-3])
        str_output += '\n\tnum_sequences={}\n)'.format(len(self._ret_inds))
        return str_output
