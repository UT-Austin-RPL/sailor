...
class ConfigGenerator(object):
    ...
    """
    def __init__(self, base_config_file, script_file, generated_config_dir=None, wandb_proj_name="debug"):
        """
        Args:
        ...
        if script_file is None:
            self.script_file = os.path.join(os.getenv("HOME"), 'tmp/tmpp.sh')
        else:
    ...
    def _name_for_experiment(self, parameter_values, parameter_value_names):
        """
    ...
    def _generate_jsons(self):
        ...
        for i in range(num_settings):
            ...
            exp_name, name_modified = self._name_for_experiment(
    ...
    def _script_from_jsons(self, json_paths):
        ...
        the input jsons.
        """
        Path("/".join(self.script_file.split("/")[:-1])).mkdir(parents=True, exist_ok=True)
        with open(self.script_file, 'w') as f:
            ...
            for path in json_paths:
                ...
                cmd = "python {train_script_path} --config {config_path}\n".format(
                    train_script_path=os.path.join(robomimic_base_path, 'robomimic/scripts/train.py'),
                    config_path=path
                )
                print()
                print(cmd)
