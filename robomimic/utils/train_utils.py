...
from robomimic.envs.wrappers import Wrapper
...
def dataset_factory(config, obs_keys, filter_by_attribute=None, dataset_path=None):
    ...
    if config.algo_name == "spl":
        from robomimic.utils.dataset import SkillLearningDataset, SAILOR_Dataset
        
        ds_kwargs["load_next_obs"] = False
        ds_kwargs["temp_dist_window_len"] = config.algo.skill.temp_dist_pred.window_len
        ds_kwargs["slowness_window_len"] = config.algo.skill.slowness.window_len
        assert dataset_path is None
        dataset = dict()
        if config.train.data_prior is not None:
            dataset["prior"] = SkillLearningDataset(hdf5_path=config.train.data_prior, **ds_kwargs)
        if config.train.data_target is not None:
            target_ds_kwargs = deepcopy(ds_kwargs)
            if config.train.target_hdf5_filter_key is not None:
                target_ds_kwargs["filter_by_attribute"] = config.train.target_hdf5_filter_key
            dataset["target"] = SkillLearningDataset(hdf5_path=config.train.data_target, **target_ds_kwargs)

        if config.algo.retrieval.enabled:
            retrieval_ds_kwargs = deepcopy(ds_kwargs)
            del retrieval_ds_kwargs["temp_dist_window_len"]
            del retrieval_ds_kwargs["slowness_window_len"]

            if config.algo.retrieval.data is not None:
                retrieval_hdf5_path = config.algo.retrieval.data
                retrieval_ds_kwargs["filter_by_attribute"] = config.algo.retrieval.filter_key
            else:
                assert config.train.data_prior is not None
                retrieval_hdf5_path = config.train.data_prior

                if config.algo.retrieval.filter_key is not None:
                    retrieval_ds_kwargs["filter_by_attribute"] = config.algo.retrieval.filter_key

            assert config.train.data_target is not None

            if config.algo.retrieval.model_ckpt_path is None:
                config.algo.retrieval.unlock()
                config.algo.retrieval.model_ckpt_path = config.algo.policy.skill_params.model_ckpt_path
                config.algo.retrieval.lock()

            from robomimic.utils.file_utils import policy_from_checkpoint
            retrieval_model = policy_from_checkpoint(
                ckpt_path=config.algo.retrieval.model_ckpt_path
            )[0].policy.skill

            dataset["retrieval"] = SAILOR_Dataset(
                ret_ds_path=retrieval_hdf5_path,
                target_ds_path=config.train.data_target,
                target_filter_key=config.train.target_hdf5_filter_key,
                retrieval_model=retrieval_model,
                num_ret_cands=config.algo.retrieval.num_ret_cands,
                num_target_samples=config.algo.retrieval.num_target_samples,
                selection_frac=config.algo.retrieval.selection_frac,
                bs=config.algo.retrieval.bs,

                **retrieval_ds_kwargs
            )

    else:
        ds_kwargs["hdf5_path"] = dataset_path if (dataset_path is not None) else config.train.data
        dataset["data"] = SequenceDataset(**ds_kwargs)
...
def run_rollout(
    ...
    assert isinstance(env, EnvBase) or isinstance(env, Wrapper)
